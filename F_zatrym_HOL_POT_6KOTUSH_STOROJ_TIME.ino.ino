...
void setup() {
  ...
  wdt_enable(WDTO_2S); // таймер сторожового догляду на 2 секунди
...
digitalWrite(ignition3, LOW); 
 ...
 if (hallValue4 == HIGH && currentMillis - previousMillis4 >= delayTime4) {
  ...
  // Перевіряємо, чи був перезапуск від сторожового таймера !!!!! ЗАБРАТИ ПРИ РОБОТІ!!!!!
  if (bit_is_set(MCUSR, WDRF)) {
    Serial.println("Перезапуск плати Arduino через сторожовий таймер!");
